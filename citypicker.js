(function (root, factory) {
  ...
  var loadDependence = function (fncallback) {
    ...
    if (!window.wwload.citypicker) {
      window.wwload.citypicker = "wait";
      // requirejs.config({
      //   paths: {
      //     "插件名": "插件路径" // 示例: libs/qrcodejs/qrcode
      //   }
      // });
      require(["libs/citypicker/cityData", "libs/citypicker/cityPicker", "css!libs/citypicker/cityPicker"], function () {
        window.wwload.citypicker = true;
        replace();
        fncallback();
      });
    } else if (window.wwload.citypicker === "wait") {
      setTimeout(function () {
  ...
  function city_picker($ele) {
    var config = {};
    config.citypickerid = $ele.attr("data-citypickerid");
    $ele.find(".cityChoice").attr("id", config.citypickerid);
    var cityPicker = new IIInsomniaCityPicker({
      data: cityData,
      target: '#' + config.citypickerid,   //城市选择元素添加的唯一的id值
      valType: 'k-v',
      // hideCityInput: '#city',
      // hideProvinceInput: '#province',
      callback: function (cid, cname) {
        // console.log(cid);
        // console.log(cname);
        if (cname == undefined) {
          $.wwclass.helper.updateProp($ele, "data-x-citypickername", "");
          $.wwclass.helper.updateProp($ele, "data-x-citypickerlng", "");
          $.wwclass.helper.updateProp($ele, "data-x-citypickerlat", "");
          $.wwclass.helper.updateProp($ele, "data-x-citypickerpoint", "");
        } else {
          $.wwclass.helper.updateProp($ele, "data-x-citypickername", cname);
          var baseUrl = 'https://api.map.baidu.com/geocoder/v2/?ak=eIxDStjzbtH0WtU50gqdXYCz&output=json&address=' + cname;
          ...
          $.ajax({
            ...
            success: function (cname) {
              if (parseInt(cname.status) == 0) {
                formdata.append("location", cname.result.location);
                formdata.append("lng", cname.result.location.lng);//cname.result.location.lng 经度
                formdata.append("lat", cname.result.location.lat);//cname.result.location.lat 维度
                $.wwclass.helper.updateProp($ele, "data-x-citypickerlng", cname.result.location.lng);
                $.wwclass.helper.updateProp($ele, "data-x-citypickerlat", cname.result.location.lat);
                $.wwclass.helper.updateProp($ele, "data-x-citypickerpoint", JSON.stringify(cname.result.location));
                // console.log(cname.result.location.lng);
                // console.log(cname.result.location.lat);
                // console.log(JSON.stringify(cname.result.location));
              }
            }
          })
        }
  ...
  function processElement($ele) {
    ...
    setTimeout(function () {
      $.wwclass.helper.updateProp($ele, "data-x-inited", true);
      $ele.find(".city_choice").removeClass("hidden");
      city_picker($ele);
    }, 100);
